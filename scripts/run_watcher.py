...
def run_watcher(
    ...
    Args:
        redis_stream_key: Base Redis stream key (will use {key}:subsampled)
    ...
    try:
        while True:
            ...
            if result["success"]:
                ...
                logger.error(f"Error processing frames: {result.get('error', 'Unknown error')}")
                time.sleep(0.3)
...
def parse_args():
    ...
    parser.add_argument(
        "--redis-stream-key", required=True, help="Base Redis stream key"
    )
    parser.add_argument(
